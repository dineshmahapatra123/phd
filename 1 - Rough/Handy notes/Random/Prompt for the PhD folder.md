
**Codex Prompt**

First read:
- CODEX.md
- .codex-phd/skills/phd-vault/SKILL.md
- 9 - Knowledge_base/PHD_CONSTITUTION.md
- 9 - Knowledge_base/PHD_SCHEMA.md
- 9 - Knowledge_base/PHD_LENS.md
- 9 - Knowledge_base/log.md

Important rules:
- Follow CODEX.md for Codex-specific command mappings and safety rules.
- Codex must remain usable even if `.claude/` and `.agent/` are deleted.
- For command-specific behaviour, read the matching `.codex-phd/workflows/<command>.md`.
- 9 - Knowledge_base/sources/ is immutable.
- Never write into ## Highlights in master notes.
- log.md is compact; detailed audits are in 9 - Knowledge_base/logs/.
- Do not load detailed audits by default. Open them only for provenance, reopened papers, ambiguous workflow status, or file-level history.
- Do not assume every Raw PDF is processed. Use log.md as the first status check.
- For style, citations, and bibliography, use 9 - Knowledge_base/TISS_STYLE.md as canonical.
- Treat older notes in `1 - Rough/Handy notes/Random/` as historical unless they agree with CODEX.md.


**Claude Prompt**

You are working inside my Obsidian PhD vault.

First read or respect:
- CLAUDE.md
- 1 - Rough/Handy notes/Random/PhD Workflow Guide.md
- 9 - Knowledge_base/PHD_CONSTITUTION.md
- 9 - Knowledge_base/PHD_SCHEMA.md
- 9 - Knowledge_base/log.md

Important rules:
- The PhD Workflow Guide is the practical workflow authority.
- 9 - Knowledge_base/sources/ is immutable.
- Never write into ## Highlights in master notes.
- log.md is compact; detailed audits are in 9 - Knowledge_base/logs/.
- Do not load detailed audits by default. Open them only for provenance, reopened papers, ambiguous workflow status, or file-level history.
- Do not assume every Raw PDF is processed. Use log.md as the first status check.
- For style, citations, and bibliography, use 9 - Knowledge_base/TISS_STYLE.md as canonical.
- Before using a slash command, read the relevant .claude/commands/ or .agent/workflows/ file.


**Antigravity / OpenClaw Prompt**

You are working inside my Obsidian PhD vault using the .agent workflow system.

First read:
- ANTIGRAVITY.md
- .agent/memory/vault_context.md
- 1 - Rough/Handy notes/Random/PhD Workflow Guide.md
- 9 - Knowledge_base/PHD_CONSTITUTION.md
- 9 - Knowledge_base/PHD_SCHEMA.md
- 9 - Knowledge_base/log.md

Important rules:
- The PhD Workflow Guide is the practical workflow authority.
- .agent/workflows/ contains the workflow procedures.
- .agent/skills/ contains custom skills such as pdf-to-markdown.
- 9 - Knowledge_base/sources/ is immutable.
- Never write into ## Highlights in master notes, to be filled manually.
- log.md is compact; detailed audits are in 9 - Knowledge_base/logs/.
- Do not load detailed audits by default. Open them only for provenance, reopened papers, ambiguous workflow status, or file-level history.
- Do not assume every Raw PDF is processed. Use log.md as the first status check.
- For style, citations, and bibliography, use 9 - Knowledge_base/TISS_STYLE.md as canonical.
- Before executing a workflow, read the matching .agent/workflows/<command>.md file.
